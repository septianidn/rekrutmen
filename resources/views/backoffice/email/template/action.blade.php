<div class="flex align-items-center list-email-template-action">
    <a class="btn btn-sm btn-icon btn-primary" data-bs-toggle="modal"
        data-bs-target="#showDetailTemplate{{ $data->id }}" title="Edit fakultas">
    ...
    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" title="Edit Template"
        href="{{ route('backoffice.template.edit', $data->id) }}">
        <span class="btn-inner">
    ...
    ?>
    @if ($data->nama_template != 'No Template')
        <a class="btn btn-sm btn-icon btn-danger" onclick="showDeleteConfirmation({{ $data->id }})"
            data-bs-toggle="tooltip" title="Delete Template">
            <span class="btn-inner">
                <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                    stroke="currentColor">
                    <path
...
<div class="modal fade" id="showDetailTemplate{{ $data->id }}" tabindex="-1" aria-labelledby="addOrUpdateDataLabel"
    aria-hidden="true">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="nama_template" value="{{ $data->nama_template }}"
                                class="form-control" placeholder="Nama Template" readonly id="nama_template">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label" for="subjek_template">Subjek</label>
                        <div class="col-sm-10">
                            <input type="text" name="subjek_template" value="{{ $data->subjek_template }}"
                                class="form-control" placeholder="Subjek Template" readonly id="subjek_template">
                        </div>
                    </div>
                    <div class="form-group row">
    ...
    function showDeleteConfirmation(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = "{{ route('backoffice.template.destroy', '') }}/" + id;
                form.method = 'POST';
