<x-app-layout :assets="$assets ?? []">
    {{-- <div class="container">
        ...
        <div id="card-container">
            <div class="card mb-3" id="card-0">
                <div class="card-body">
                    <h5 class="card-title">Card Title 1</h5>
                    <input class="card-text">
                    <button class="btn btn-primary add-card">Tambah Card</button>
                    <button class="btn btn-danger remove-card">Hapus Card</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            // Fungsi untuk menambahkan card baru di bawah elemen yang sesuai
            function addCard($element) {
                var cardCount = $element.siblings('.card').length;
                var card = `<div class="card mb-3">
                                <div class="card-body">
                                    <h5 class="card-title">Card Title ${cardCount + 1}</h5>
                                    <input class="card-text">
                                    <button class="btn btn-primary add-card">Tambah Card</button>
                                    <button class="btn btn-danger remove-card">Hapus Card</button>
                                </div>
            ...
            // Fungsi untuk mengupdate ulang indeks
            function updateCardIndex() {
                $("#card-container .card").each(function(index) {
                    $(this).find('.card-title').text(`Card Title ${index + 1}`);
                });
            }


            // Tambahkan card saat tombol "Tambah Card" di dalam card diklik
            $(document).on("click", ".add-card", function() {
                var $parentCard = $(this).closest('.card');
                addCard($parentCard);
            });

            // Hapus card saat tombol "Hapus Card" di dalam card diklik
    ...
    $(document).ready(function() {
        ...
        function addPage() {
            if (currentPage < maxPages) {
                ...
                $(`#page\\[${currentPage}\\]`).addClass("padding-10");
                currentPage++;
            ...
            <div class="form-card text-start">
                <div class="row">
                    <div class="col-sm-12 col-lg-12" id="card_container[${page}]">
                        <input type="text" class="h3 px-2 mb-4 dynamic-input" id="nama_halaman[${page}]" value="" style="transition: width 0.2s;" >
        ...
        <div class="d-flex float-end">
            <a class="mx-1 delete_card" type="button" id="delete_card[0]">
                <svg width="21" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        ...
                        stroke="#009A4B" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path d="M20.708 6.23975H3.75" stroke="#009A4B" stroke-width="1.5" stroke-linecap="round"
                ...
                <svg width="20px" height="20px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <g id="SVGRepo_iconCarrier">
                        ...
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"
                            sketch:type="MSPage">
                            <g id="Icon-Set" sketch:type="MSLayerGroup" transform="translate(-204.000000, -931.000000)"
                                fill="#009A4B">
                                <path
            ...
            <div class="dropup mx-2 more_dropdown">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenu[0]" role="menu">
                    <li><a class="dropdown-item add_heading" id="add_heading[0]" type="button"><svg fill="#009a4b"
                                width="20px" height="20px" viewBox="0 0 24.00 24.00" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <li><a class="dropdown-item add_pertanyaan" id="add_pertanyaan[0]" type="button"><svg width="18px"
...
<script>
    ...
    function rangeListener() {
        ...
        function labelRangeCounter() {
            ...
            for (var i = counterStartValue; i <= endValue; i++) {
                var labelRange = `
                <div class="input-group range-label-group">
    ...
    function addCard(indexCard, cardSelector) {
        ...
        if (cardAfter.length > 0) {
            // If cardSoalAfter is found, insert the new card right before it
            cardAfter.before(addCard);
            resetIndex();

            // Mengambil cardId berdasarkan cardAfter
            const newCardId = cardAfter.attr('data-id');
            initializeSelect2(newCardId);
        } else {
            // If cardSoalAfter is not found, insert the new card after cardSoalBefore
            cardBefore.after(addCard);
            resetIndex();

            // Mengambil cardId berdasarkan cardBefore
            const newCardId = cardBefore.attr('data-id');
            initializeSelect2(newCardId);
        }
