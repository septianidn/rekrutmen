...
function gridColumnListener() {
    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('grid-delete-row')) {
            var rowContainer = event.target.closest('.grid-row');
            var rowIndex = Array.from(document.querySelectorAll('.grid-row')).indexOf(rowContainer);

            rowContainer.remove();

            // Hapus elemen nomor yang berkaitan berdasarkan indeks baris
            var gridNumberDivs = Array.from(document.querySelectorAll(
                '.grid-number-container .grid-number-div'));
            gridNumberDivs[rowIndex].remove();

            updateRowNumbers();
            updateNumbers();
        }

        if (event.target.classList.contains('grid-delete-column')) {
            event.target.closest('.grid-column').remove();
            updateColumnNumbers();
        }
    });

    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('grid-add-row-button')) {
            var newRowNumber = document.querySelectorAll('.grid-row-container .grid-row').length + 1;
            var newElementRow = document.createElement('div');
            newElementRow.className = 'row my-2 grid-row';
            newElementRow.innerHTML = `
                    <div class="col-lg-12">
                        <div class="input-group">
                            <input type="text" class="form-control grid-row-input" placeholder="Label Baris ${newRowNumber}">
                            <input type="text" class="form-control grid-row-input-value" placeholder="Nilai Baris ${newRowNumber}">
                ...
                `;
            document.querySelector('.grid-row-container').appendChild(newElementRow);

            var newElementNumber = document.createElement('div');
            newElementNumber.className = 'row my-2 align-items-center grid-number-div';
            newElementNumber.setAttribute('data-row-number', newRowNumber);
            newElementNumber.innerHTML = `
                    <p class="text-center grid-number">${newRowNumber}</p>
                `;
            document.querySelector('.grid-number-container').appendChild(newElementNumber);
    ...
    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('grid-add-column-button')) {
            var newElementRow = document.createElement('div');
            newElementRow.className = 'row my-2 grid-column';
            newElementRow.innerHTML = `
                    <div class=col-lg-12">
                        <div class="input-group">
                            <input type="text" class="form-control grid-column-input" placeholder="Label Kolom">
                            <input type="text" class="form-control grid-column-input-value" placeholder="Nilai Kolom">
                ...
                `;
            document.querySelector('.grid-column-container').appendChild(newElementRow);
    ...
    function updateRowNumbers() {
        var gridRowElements = document.querySelectorAll('.grid-row-container .grid-row');
        gridRowElements.forEach(function (element, index) {
            var rowNumber = index + 1;
            element.querySelector('.grid-row-input').placeholder = "Label Baris " + rowNumber;
            element.querySelector('.grid-row-input-value').placeholder = "Nilai Baris " + rowNumber;
        });
    }

    function updateNumbers() {
        var gridNumberDivs = document.querySelectorAll('.grid-number-container .grid-number-div');
        gridNumberDivs.forEach(function (element, index) {
            element.querySelector('.grid-number').textContent = index + 1;
        });
    }

    function updateColumnNumbers() {
        var gridColumnElements = document.querySelectorAll('.grid-column-container .grid-column');
        gridColumnElements.forEach(function (element, index) {
            var columnNumber = index + 1;
            element.querySelector('.grid-column-input').placeholder = "Label Kolom " + columnNumber;
            element.querySelector('.grid-column-input-value').placeholder = "Nilai Kolom " + columnNumber;
        });
    }
...
function checkBoxListener() {
    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('add-checkbox-option')) {
            var newElementCheckbox = document.createElement('div');
            newElementCheckbox.className = 'checkbox-option py-1';
            newElementCheckbox.id = 'checkbox';
            newElementCheckbox.innerHTML = `
                ...
                `;
            event.target.closest('.checkbox-option').after(newElementCheckbox);
        }

        if (event.target.classList.contains('delete-checkbox-option')) {
            event.target.closest('.checkbox-option').remove();
        }
    });
}
...
function radioBoxListener() {
    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('add-radio-option')) {
            var newElement = document.createElement('div');
            newElement.className = 'radio-option py-1';
            ...
            newElement.innerHTML = `
                ...
                `;
            event.target.closest('.radio-option').after(newElement);
        }

        if (event.target.classList.contains('delete-radio-option')) {
            event.target.closest('.radio-option').remove();
        }
...
function radioBoxAdditionalListener() {
    document.addEventListener('change', function (event) {
        if (event.target.classList.contains('singlechoice-addition')) {
            var singlechoiceAdditionText = event.target.closest('.radio-option').querySelector(
                '.singlechoice-addition-text');
            if (event.target.checked) {
                singlechoiceAdditionText.style.visibility = 'visible';
            } else {
                singlechoiceAdditionText.style.visibility = 'hidden';
            }
        }
    });
}
...
function initialize() {
    // document.querySelector("#card_container").appendChild(generateHeading(0));
    document.querySelector("#card_container").insertAdjacentHTML('beforeend', generateCard(0));
    listenerCard();
}

function listenerCard() {

    const cardSoals = document.querySelectorAll('.card-soal');
    cardSoals.forEach(function (cardSoal) {
        cardSoal.addEventListener('click', function () {
            this.classList.add('active');
            const siblings = this.parentElement.querySelectorAll('.card-soal:not(.active)');
    ...
    const addPertanyaans = document.querySelectorAll('.add_pertanyaan');
    addPertanyaans.forEach(function (addPertanyaan) {
        addPertanyaan.addEventListener('click', function () {
    ...
    const addHeadings = document.querySelectorAll('.add_heading');
    addHeadings.forEach(function (addHeading) {
    ...
    const duplicateCards = document.querySelectorAll('.duplicate_card');
    duplicateCards.forEach(function (duplicateCard) {
        duplicateCard.addEventListener('click', function () {
            const id = this.getAttribute('id');
            const index = id.match(/\d+/)[0];
    ...
    const deleteCards = document.querySelectorAll('.delete_card');
    deleteCards.forEach(function (deleteCard) {
        ...
        tinymce.init({
            ...
            setup: function (ed) {
                ed.on("click", function () {
                    ...
                    cardSoal.classList.add("active");
                    const
                        siblings = cardSoal.parentElement.querySelectorAll(
                            '.card-soal:not(.active)');
                    siblings.forEach(function (sibling) {
                        sibling.classList.remove("active");
                    });
                });
...
function initializeSelect2() {
    ...
    $('.type-jawaban-select').each(function () {
        ...
        var cardId = selectElement.data('card');

        (function (cardId) {
            selectElement.on('change', function () {
                toggleInputs(cardId); // Now, it captures the correct cardId
            });
        })(cardId); // Pass cardId as an argument to the IIFE
...
function addCard(indexCard, cardSelector) {
    ...
    // Update the indexes of all cards
    resetIndex();
}
...
function formatState(opt) {
    ...
    var optimage = opt.element.getAttribute('data-image');
    if (!optimage) {
        return opt.text;
    } else {
        var $opt = document.createElement('span');
        $opt.innerHTML = optimage + opt.text;
        return $opt;
    }
}
