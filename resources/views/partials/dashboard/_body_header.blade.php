    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav ms-auto  navbar-list mb-2 mb-lg-0">
            ...
            <div class="card shadow-none m-0">
              <div class="card-header d-flex justify-content-between bg-primary py-3">
                <div class="header-title">
                  <h5 class="mb-0 text-white">Notifikasi ({{ $notifCount }})</h5>
                </div>
                @if($notifCount > 0)
                <form action="{{ route('notification.mark-all-read') }}" method="POST">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-light py-0 px-2">Tandai semua dibaca</button>
                </form>
                @endif
              </div>
              <div class="card-body p-0" style="max-height: 350px; overflow-y: auto;">
                @forelse($notifications as $notif)
                <a href="{{ route('notification.read', $notif->id) }}" class="iq-sub-card {{ !$notif->is_read ? 'bg-soft-primary' : '' }}">
                  <div class="d-flex align-items-center">
                    <div class="ms-3 w-100">
                      <h6 class="mb-0">{{ $notif->title }}</h6>
                      <div class="d-flex justify-content-between align-items-center">
                        <p class="mb-0 text-truncate" style="max-width: 200px;">{{ $notif->message }}</p>
                        <small class="float-right font-size-12">{{ $notif->created_at->diffForHumans() }}</small>
                      </div>
                    </div>
                  </div>
                </a>
        ...
        <li class="nav-item dropdown">
          ...
          $profileImage = auth()->user()->getFirstMedia('profile_image') ?? null;
          @endphp
          <a class="nav-link py-0 d-flex align-items-center" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            @if ($profileImage)
              <img src="{{ $profileImage->getUrl() ?? asset('images/avatars/01.png') }}" alt="User-Profile" class="theme-color-default-img img-fluid avatar avatar-50 avatar-rounded">
           @else
